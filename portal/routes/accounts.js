...
module.exports = function (app, request, ports) {
    app.post('/endpoints/accounts/transfer', function (req, res) {
        ...
        var options = {
            method: 'POST',
            uri: `${req.protocol}://${process.env.BASE_PATH}:${ports.transactions}${process.env.CREATE_TRANSACTION_ENDPOINT}`,
            body: params,
            json: true
        };
        request.post(options, function (error, response, body) {
            ...
            var options = {
                 method: 'POST',
                 uri: `${req.protocol}://${process.env.BASE_PATH}:${ports.accounts}${process.env.ACCOUNT_WITHDRAW_ENDPOINT}`,
                 body: {
                   uuid: req.session.user.uuid,
                   amount: req.body.amount,
                   number: from
            ...
            request.post(options, function (error, response, body) {
              ...
              var options = {
                method: 'POST',
                uri: `${req.protocol}://${process.env.BASE_PATH}:${ports.accounts}${process.env.ACCOUNT_DEPOSIT_ENDPOINT}`,
                body: {
                  uuid: req.session.user.uuid,
                  amount: req.body.amount,
                  number: to
    ...
    app.get('/endpoints/accounts/activate', function (req, res) {
      ...
      var options = {
        method: 'POST',
        uri: `${req.protocol}://${process.env.BASE_PATH}:${ports.accounts}${process.env.CREATE_ACCOUNT_ENDPOINT}`,
        body: body,
        json: true
      };
      request.post(options, function (error, response, body) {
    ...
    app.post('/endpoints/accounts/get', function (req, res) {
      var options = {
        method: 'POST',
        uri: `${req.protocol}://${process.env.BASE_PATH}:${ports.accounts}${process.env.GET_ACCOUNTS_ENDPOINT}`,
        body: {
          uuid: req.session.user.uuid
        },
        json: true
